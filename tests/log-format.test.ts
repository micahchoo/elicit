...
{ kind: 'thread-deferred', detail: `thread=${ULID} strikes=2`, reads: 'deferred a thread after two disengaged replies' },
{
 ...
 // emitter writes, so they pass through verbatim rather than being reworded.
 {
  ...
  detail: 'mode=shadow threshold=lint.godNodeFanout value=6 would=note god-node on referent=atlas claims=9 over fanout=6',
  reads: 'did not act on lint.godNodeFanout, set to 6 — it would note god-node on referent=atlas claims=9 over fanout=6',
 },
 {
