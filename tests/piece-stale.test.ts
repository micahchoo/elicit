...
import type { Entry, Marginalia } from '../src/piece/contract.js';
...
describe('stalePins', () => {
  it('flags a pin to an older version once, on the entry id, with no model', () => {
    const entries: Entry[] = [
      ...
      { id: 'e2', placedBy: 'person' },
    ];
    const found = stalePins(entries, { 's-a': v3 });
  ...
  it('flags nothing for a pin to the current version', () => {
    const entries: Entry[] = [{ id: 'e1', kind: 'pin', snippet: 's-a', version: 3 }];
    expect(stalePins(entries, { 's-a': v3 })).toEqual([]);
  });

  it('does not mutate the entries — in particular no pin version changes', () => {
    const entries: Entry[] = [
      { id: 'e1', kind: 'pin', snippet: 's-a', version: 1 },
      { id: 'e2', kind: 'pin', snippet: 's-a', version: 2 },
    ];
    const before = structuredClone(entries);
    stalePins(entries, { 's-a': v3 });
  ...
  it('is pure and memoryless: same findings, same order, on every call', () => {
    ...
    const first = stalePins(entries, snippets);
    const second = stalePins(entries, snippets);
  ...
  it('yields nothing for a gap entry', () => {
    const entries: Entry[] = [{ id: 'e1', placedBy: 'person' }];
    expect(stalePins(entries, { 's-a': v3 })).toEqual([]);
  });
  ...
  it('exports nothing taking a Complete', () => {
    ...
    const calls = () => {
      // @ts-expect-error stalePins takes an Entry list, never a Complete
