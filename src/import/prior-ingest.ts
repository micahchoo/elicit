/**
 * The one-off ingest script's decisions, as data — ticket 058, Task 8.
 *
 * `scripts/ingest-posts.ts` runs TOP-LEVEL CODE on import — it reads argv and
 * exits when run without --dry/--apply — so its manifest cannot be imported
 * from. This module holds the decisions that script was built on, verbatim:
 * the 19 posts kept (`MANIFEST`) and the 12 refusal groups (`EXCLUDED`) with
 * the reason each item stayed out. The script imports them from here, and
 * the adoption step (`src/import/adopt.ts`) reads them to reconcile the
 * staging store with what the one-off already decided.
 *
 * Verbatim move: `selectBody` in the script still consumes `Select`, and the
 * reasons are the reader's words — do not reword them.
 */
export type Select =
...
export const EXCLUDED: { slug: string; why: string }[] = [
