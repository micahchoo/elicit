...
import { SUPERSEDE_MODEL_UPGRADE, shadowCollector } from '../wiki/contract.js';
...
import type { ConfirmResult, OppositionJudgment } from './contradiction.js';
import { readSitting, sittingCache } from './sitting.js';
import { composeDiscriminatingQuestion } from './composed.js';
...
import {
 confirmAnsweredRemeasures,
 dissolve,
 jobRangeDiscrimination,
 loadWorld,
 recoverPoles,
} from './confirmation.js';

export { dissolutionOutcome, RANGE_DISCRIMINATED } from './confirmation.js';
...
function originalQuestions(a: Claim, b: Claim, graph: ClaimGraph): string[] {
 ...
 for (const cite of [...a.cites, ...b.cites]) {
  const question = graph.snippets[citeSnippetId(cite)]?.provenance.question;
...
async function jobLint(
 ...
 const { graph } = loadWorld(deps, graphOf);
 ...
 for (const finding of findings) {
  ...
  const stale = finding.refs
   .slice(1)
   .map((ref) => graph.snippets[citeSnippetId(ref)])
